...
int randomMethodLevel3(int max)
{
	int randomNo = rand();
...
GameLevel3::GameLevel3(GameDataRef data) : _data(data)
{
...
void GameLevel3::Init()
{
	...
	LuaRef lv3 = getGlobal(F, "level3");
	LuaRef lvl3bull = lv3["level3AllBullets"];
	LuaRef lvl3finpt = lv3["level3FinalPoints"];
	LuaRef lvl3enmIS = lv3["level3EnemyInitialSize"];
	LuaRef lvl3enmFS = lv3["level3EnemyFinalSize"];
	LuaRef lvl3PFHAN = lv3["level3PointsForHittingAnEnemy"];
	LuaRef lvl3HT = lv3["level3HealthTakenByEnemy"];

	level3AllBullets = lvl3bull.cast<int>();
	level3FinalPoints = lvl3finpt.cast<int>();
	level3EnemyInitialSize = lvl3enmIS.cast<int>();
	level3EnemyFinalSize = lvl3enmFS.cast<int>();
	level3PointsForHittingAnEnemy = lvl3PFHAN.cast<int>();
	level3HealthTakenByEnemy = lvl3HT.cast<int>();
	...
	_player._weapon._text.setString("BULLETS		" + to_string(_player._weapon.bulletsUsed) + "	OF	" + to_string(LEVEL_3_ALL_BULLETS));
	_player._weapon._textPoints.setString("POINTS		" + to_string(_player._weapon.points) + "	OF	" + to_string(LEVEL_3_FINAL_POINTS));
...
void GameLevel3::Update(float delta)
{
	...
	if (_player._weapon.bulletsUsed < LEVEL_3_ALL_BULLETS)
	{
		...
		if (elapsed.asSeconds() >= 0.11)
		{
			...
			if (Keyboard::isKeyPressed(Keyboard::Space))
			{
				...
				if (enemyArray.size() < LEVEL_3_ENEMY_FINAL_SIZE)
				{
					...
					_enemy._playerBD.position.Set(randomMethodLevel3(8), -randomMethodLevel3(5));
					_enemy._playerMove = world.CreateBody(&_enemy._playerBD);
					_enemy._playerMove->CreateFixture(&_enemy.fixtureDef);
	...
	for (_player.iter = _player.weaponArray.begin(); _player.iter != _player.weaponArray.end(); _player.iter++)
	{
		...
		{
			if (_player.weaponArray[counterLevel3].missle.getGlobalBounds().intersects(enemyArray[counterLevel31]._sprite.getGlobalBounds()))
			{
				_player.weaponArray[counterLevel3].deleteBullets = true;
				_player._weapon.points += LEVEL_3_POINTS_FOR_HITTING_AN_ENEMY;
			}
			counterLevel31++;
		}
		counterLevel3++;
	...
	for (_player.iter = _player.weaponArray.begin(); _player.iter != _player.weaponArray.end(); _player.iter++)
	{
		if (_player.weaponArray[counterLevel3].deleteBullets == true)
		{
	...
	{
		if (_player.health > 0)
			_player.health -= LEVEL_3_HEALTH_TAKEN;
		else
			_player.health = 0;
	...
	if (_player._weapon.points >= LEVEL_3_FINAL_POINTS)
		this->_data->machine.AddState(StateRef(new WinningState(_data)), true);
	...
	if (vertical == false)
	{
		...
		{
			//the posiiton is random for the whole width and height of the screen and there is a body and fixture created for each of the so the colliders could work
			_wall.groundBodyDef.position.Set(randomMethodLevel3(8), -randomMethodLevel3(5));
			_wall.groundBody = world.CreateBody(&_wall.groundBodyDef);
			_wall.groundBody->CreateFixture(&_wall.groundBox, 0.0f);
	...
	if (horizontal == false)
	{
		...
		{
			//the posiiton is random for the whole width and height of the screen and there is a body and fixture created for each of the so the colliders could work
			_hoWall.groundBodyDef.position.Set(randomMethodLevel3(9), -randomMethodLevel3(5));
			_hoWall.groundBody = world.CreateBody(&_hoWall.groundBodyDef);
			_hoWall.groundBody->CreateFixture(&_hoWall.groundBox, 0.0f);
	...
	if (enemyInitialise == false)
	{
		...
		//when it reached this array size it should stop making them
		if (enemyArray.size() > LEVEL_3_ENEMY_INITIAL_SPWAN_SIZE)
...
void GameLevel3::Draw(float delta)
{
	...
	for (_player.iter = _player.weaponArray.begin(); _player.iter != _player.weaponArray.end(); _player.iter++)
	{
		_player.weaponArray[counterLevel3].Update(delta);
		this->_data->window.draw(_player.weaponArray[counterLevel3].missle);
		counterLevel3++;
	}
	counterLevel31 = 0;
	for (iterEnemy = enemyArray.begin(); iterEnemy != enemyArray.end(); iterEnemy++)
	{
		enemyArray[counterLevel31].Update(delta);
		this->_data->window.draw(enemyArray[counterLevel31]._sprite);
		counterLevel31++;
	}
	counterLevel32 = 0;
	for (_wall.iterWall = _wall.wallArray.begin(); _wall.iterWall != _wall.wallArray.end(); _wall.iterWall++)
	{
		_wall.wallArray[counterLevel32].Update(delta);
		this->_data->window.draw(_wall.wallArray[counterLevel32].rect);
		counterLevel32++;
	}
	counterLevel33 = 0;
	...
	{
		_hoWall.wallHorizontalArray[counterLevel33].Update(delta);
		this->_data->window.draw(_hoWall.wallHorizontalArray[counterLevel33].rect);
		counterLevel33++;
	}
